		...
		if (null != list && list.size() > 0) {
			...
			if (linkList != null && linkList.size() > 0) {
				for (int i = 0; i < linkList.size(); i++) {
					...
					String str = link.getLink().replace("view_text_img(", "").replace(")", "").replace("'", "");
					if(null != str && str.length() > 0){
						...
						body = HttpClientUtils.getBody(PREFIX_SATELINE_CLOUD_IMG_URL+tmps[0]);
						logger.info("\t小图["+name+"],时间["+date+"],大小:"+body.length);
						
						
						name = analysisURL(tmps[1]);
						date = analysisURL2(name);
						body = HttpClientUtils.getBody(PREFIX_SATELINE_CLOUD_IMG_URL+tmps[1]);
						logger.info("\t大图["+name+"],时间["+date+"],大小:"+body.length);
					}
					logger.info("\r\n");
				}
			}
