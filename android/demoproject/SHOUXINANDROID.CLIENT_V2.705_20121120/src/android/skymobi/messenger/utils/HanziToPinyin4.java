...
public class HanziToPinyin4 {
    private static final String TAG = "HanziToPinyin4";

    // Turn on this flag when we want to check internal data structure.
    private static final boolean DEBUG = false;

    private final boolean isLowerLetter = true;
    /**
        ...
        synchronized (HanziToPinyin4.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    ...
                    if (DEBUG) {
                        SLog.d(TAG, "Self validation. Result: " + doSelfValidation());
                    }
            ...
            sInstance = new HanziToPinyin4(false);
            return sInstance;
        }
    }
    ...
    public Token getToken(char character) {
        ...
        for (int j = 0; j < PINYINS[offset].length && PINYINS[offset][j] != 0; j++) {
            if (isLowerLetter) {
                pinyin.append((char) (PINYINS[offset][j] + 32));
            } else {
    ...
    private void addToken(
        ...
        if (isLowerLetter) {
            tokens.add(new Token(tokenType, str, str.toLowerCase()));
        } else {
